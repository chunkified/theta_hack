...
class PTP_IP(object):
    ...
    # -------------------------------------------------------------------------
    def InitiateOpenCapture(self):
        print 'Send PTP_OC_InitiateOpenCapture'
        #print result
        self.Send_PTPCommandRequest(self.command_sock, self.transaction_id,
                                    '', PTP_OC_InitiateOpenCapture, 0, 0)
        self.transaction_id += 1
        result, args, payload = self.Wait_PTPCommandResponse(self.command_sock)
        if result == 0:
            print 'Failed'
    ...
    # -------------------------------------------------------------------------
    def TerminateOpenCapture(self):
        print 'Send PTP_OC_TerminateOpenCapture'
        #print result
        self.Send_PTPCommandRequest(self.command_sock, self.transaction_id,
                                    '', PTP_OC_TerminateOpenCapture, self.transaction_id-1, 0)
        #引数として与える停止対象動画のtransaction_idはInitiateOpenCapture時に保存したものを使うべき
        #今回は動画は一つしか撮影しないので、単純に今のtransaction_idの一つ前を指定している
